namespace Chirp.Web.Playwright.Test
{
    ...
    class EndToEndTests
    {
        ...
        {
            if (TestContext.CurrentContext.Test.Name != nameof(UsersCanRegisterAndLogin))
            {
                _context = await _browser.NewContextAsync();
                _page = await _context.NewPageAsync();

                await _page.GotoAsync("http://localhost:5273/Identity/Account/Login");
                await _page.GetByPlaceholder("name@example.com").FillAsync("jing@itu.dk");
                await _page.GetByPlaceholder("password").FillAsync("Johan1234!");
                await _page.GetByRole(AriaRole.Button, new() { Name = "Log in" }).ClickAsync();
                await _page.WaitForURLAsync("http://localhost:5273/");
            }
        }
